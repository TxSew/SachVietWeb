...
function Checkout() {
    ...
    const fetchProvince = async () => {
        ...
        axios.get('https://provinces.open-api.vn/api/?depth=2').then((res) => {
            const getDistrictsByProvince = (provinceCode: any) => {
                const province = res.data.find((province: any) => province.code === provinceCode);
                if (province) {
            ...
            // Sử dụng hàm để lấy thông tin quận/huyện theo tỉnh Hà Giang
            const haGiangDistricts = getDistrictsByProvince(1);
            // setDistrict(haGiangDistricts);
                    ...
                    >
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Select {...field} displayEmpty inputProps={{ 'aria-label': 'Without label' }}>
                                        {province.map((e: any) => {
                                            return (
                                                <MenuItem key={e.code} value={e.id}>
                                                    {e.name}
                                                </MenuItem>
                                            );
                                        })}
                        ...
                        <FormControl>
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Select {...field} displayEmpty inputProps={{ 'aria-label': 'Without label' }}>
                                        {province[1]?.districts?.map((e: district) => {
                                            return (
                                                <MenuItem value={e.id}>
                                                    <em>{e.name}</em>
                                                </MenuItem>
